...
- **FR-009**: If feature detection fails (contract supports neither AccessControl nor Ownable), the system MUST NOT persist any contract data and MUST display a clear error message explaining why the contract cannot be managed. Any temporary UI state (form fields, loading indicators) MUST be reset to allow the user to try a different contract.
- **FR-010**: If network connectivity is lost during a mutation operation, the system MUST detect the disconnection, halt the operation with a clear "Network disconnected" error, and preserve the user's input state so they can retry when connectivity is restored. The UI MUST NOT show success or leave the user in an ambiguous state.
- **FR-011**: If the user rejects a wallet signature request during a mutation, the system MUST catch the rejection error, display a non-blocking notification (e.g., "Transaction cancelled by user"), and return the UI to its pre-mutation state without data loss.
- **FR-012**: If a contract passes initial validation but subsequent service calls (e.g., `getCurrentRoles`, `grantRole`) fail with adapter errors, the system MUST display a contextual error message (e.g., "Unable to fetch roles for this contract") without crashing. The contract record MAY remain persisted, but the affected feature tab MUST show an error state with a "Retry" option.
- **FR-013**: The `useContractRoles` hook MUST support pagination parameters (offset/limit or cursor-based) if the adapter provides them. For adapters without pagination support, the hook MUST fetch all data but the UI SHOULD implement client-side virtualization for lists exceeding 100 items to maintain performance.
- **FR-014**: After a successful mutation, the system MUST invalidate and refetch the affected data (roles or ownership) to reflect the current on-chain state. The UI SHOULD also support manual refresh and MAY implement automatic polling (configurable interval) for active sessions to detect external changes.
